...
export default function BookChatModal({ isOpen, onClose, bookTitle, authors }: BookChatModalProps) {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      setMessages([
        { 
          role: 'ai', 
          content: `Ask me anything about "${bookTitle}" - plot, characters, themes, or anything else!` 
        }
      ])
    }
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/books/chat', {
        title: bookTitle,
        authors: authors,
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-4 border-b bg-gradient-to-r from-purple-600 to-indigo-600 text-white flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-sm">Chat with Book</h3>
              <p className="text-xs text-white/80 line-clamp-1">{bookTitle}</p>
            </div>
          </div>
          <button 
            onClick={onClose}
